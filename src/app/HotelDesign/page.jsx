...
const HotelDesign = ({ hotel }) => {
  const { img, location, available } = hotel;
  ...
  return (
    <div className="border rounded-lg shadow-md overflow-hidden bg-white hover:shadow-lg transition">
      ...
      <div className="p-4 justify-between flex">
        <h2 className="text-lg font-semibold flex items-center space-x-2"><IoLocationSharp /> {location}</h2>
        <button className="text-white text-base bg-black px-6 py-1 rounded-md cursor-pointer">Book Now</button>
      </div>
    </div>
  );
};
